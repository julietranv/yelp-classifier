...
class YelpClassifier(object):
    ...
    def calculate_likelihoods(self, train_rdd):
        ...
        class_likelihoods = train_rdd \
                             .flatMap(lambda t: [((t[1], word), 1) for word in t[2].split(" ")]) \
			     .reduceByKey(lambda v1, v2: v1 + v2) \
                             .map(lambda t: (t[0][0], {t[0][1]: self.compute_likelihood(t[1], float(t[0][0]))})) \
                             .reduceByKey(lambda v1, v2: self.combine_probability_tables(v1, v2))
    ...
    def calculate_num_reviews_and_words_per_num_stars(self, train_rdd):
        ...
        num_reviews_words_per_num_stars = train_rdd \
                                             .map(lambda t: (t[1], (1, len(t[2].split(" "))))) \
    ...
    def classify_reviews(self, test_rdd):
        ...
        predictions = test_rdd \
                            ...
                            .reduceByKey(lambda v1, v2: self.add_log_likelihoods(v1, v2)) \
                            .map(self.likelihood_to_posterior) \
                            .map(self.review_id_only_as_key) \
                            .reduceByKey(lambda v1, v2: self.find_max_posterior(v1, v2)) \
                            .sortByKey()
    ...
    def find_max_posterior(num_stars_posterior1, num_stars_posterior2):
        ...
        if float(num_stars_posterior1[1]) >= float(num_stars_posterior2[1]):
